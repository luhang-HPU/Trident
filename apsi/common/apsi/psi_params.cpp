...
namespace apsi {
    void PSIParams::initialize()
    {
        ...
        // Create a PoseidonContext (with expand_mod_chain == false) to check validity of parameters
        PoseidonContext context =
            PoseidonFactory::get_instance()->create_poseidon_context(poseidon_params_);
        if (!context.parameters_literal()) {
            stringstream ss;
            ss << "Poseidon parameters are invalid: ";
            //    << context.parameter_error_message();
            throw invalid_argument(ss.str());
        }
        // if (!context.key_context_data()->qualifiers().using_batching) {
        //     throw invalid_argument(
        //         "Poseidon parameters do not support batching; plain_modulus must be a prime "
        //         "congruent to 1 modulo 2*poly_modulus_degree");
        // }

        // Compute the bit-length of an item
        item_bit_count_per_felt_ =
            static_cast<uint32_t>(poseidon_params_.plain_modulus().bit_count() - 1);
        ...
        items_per_bundle_ =
            static_cast<uint32_t>(poseidon_params_.degree()) / item_params_.felts_per_item;

        // Can we fit even one item into the SEAL ciphertext?
        if (!items_per_bundle_) {
            throw invalid_argument("degree is too small");
        }
    ...
    size_t PSIParams::save(ostream &out) const
    {
        ...
        vector<poseidon_byte> temp;
        temp.resize(safe_cast<size_t>(poseidon_params_.save_size(compr_mode_type::zstd)));
        size_t size = static_cast<size_t>(
            poseidon_params_.save(temp.data(), temp.size(), compr_mode_type::zstd));
        auto poseidon_params_data =
            fbs_builder.CreateVector(reinterpret_cast<const uint8_t *>(temp.data()), size);
        auto poseidon_params = fbs::CreatePoseidonParams(fbs_builder, poseidon_params_data);
        ...
        psi_params_builder.add_poseidon_params(poseidon_params);
    ...
    pair<PSIParams, size_t> PSIParams::Load(istream &in)
    {
        ...
        PSIParams::PoseidonParams poseidon_params;
        auto &poseidon_params_data = *psi_params->poseidon_params()->data();
        try {
            poseidon_params.load(
                reinterpret_cast<const poseidon_byte *>(poseidon_params_data.data()),
                poseidon_params_data.size());
        ...
        if (poseidon_params.scheme() != SchemeType::BFV) {
            throw runtime_error("failed to load parameters: invalid scheme type");
        }

        return { PSIParams(item_params, table_params, query_params, poseidon_params), in_data.size() };
    }
#ifndef APSI_DISABLE_JSON
    PSIParams PSIParams::Load(const string &in)
    {
        ...
        // PSIParams::PoseidonParams poseidon_params;
        // try {
        const auto &json_poseidon_params = get_non_null_json_value(root, "poseidon_params");
        const auto &coeff_modulus_bits =
            get_non_null_json_value(json_poseidon_params, "coeff_modulus_bits");

        size_t poly_modulus_degree = json_value_ui64(json_poseidon_params, "poly_modulus_degree");
        PSIParams::PoseidonParams poseidon_params(poly_modulus_degree);
        // poseidon_params.set_poly_modulus_degree(poly_modulus_degree);

        if (json_poseidon_params.isMember("plain_modulus") &&
            json_poseidon_params.isMember("plain_modulus_bits")) {
            throw runtime_error(
                "only one of plain_modulus and plain_modulus_bits must be specified");
        }
        if (json_poseidon_params.isMember("plain_modulus")) {
            poseidon_params.set_plain_modulus(json_value_ui64(json_poseidon_params, "plain_modulus"));
        } else if (json_poseidon_params.isMember("plain_modulus_bits")) {
            poseidon_params.set_plain_modulus(PlainModulus::Batching(
                poly_modulus_degree, json_value_int(json_poseidon_params, "plain_modulus_bits")));
        } else {
        ...
        for (const auto &coeff : coeff_modulus_bits) {
            ...
            // poseidon_params.set_coeff_modulus(
            //     CoeffModulus::Create(poly_modulus_degree, coeff_modulus_bit_sizes));

        // } catch (const exception &ex) {
        //     APSI_LOG_ERROR("Failed to load poseidon_params from JSON string: " << ex.what());
        //     throw;
        // }

        return PSIParams(item_params, table_params, query_params, poseidon_params);
    }
#else
    PSIParams PSIParams::Load(const string &in)
    {
    ...
    string PSIParams::to_string() const
    {
        ...
        ss << "item_params.felts_per_item: " << item_params_.felts_per_item
           ...
           << "; poseidon_params.poly_modulus_degree: " << poseidon_params_.degree()
           << "; poseidon_params.coeff_modulus: "
           << util::to_string(
                  poseidon_params_.coeff_modulus(),
                  [](const Modulus &mod) { return std::to_string(mod.bit_count()); })
           << "; poseidon_params.plain_modulus: " << poseidon_params_.plain_modulus().value();
