...
namespace apsi {
    ...
    namespace network {
        namespace {
            ...
            template <typename T>
            size_t load_from_string(string data, shared_ptr<PoseidonContext> context, T &obj)
            {
                ArrayGetBuffer agbuf(
                    reinterpret_cast<const char *>(data.data()),
                    static_cast<streamsize>(data.size()));
        ...
        unique_ptr<ZMQSenderOperation> ZMQChannel::receive_network_operation(
            shared_ptr<PoseidonContext> context, bool wait_for_message, SenderOperationType expected)
        {
            throw_if_not_connected();

            bool valid_context = context && context->parameters_literal();
            if (!valid_context && (expected == SenderOperationType::sop_unknown ||
                                   expected == SenderOperationType::sop_query)) {
                // Cannot receive unknown or query operations without a valid PoseidonContext
                APSI_LOG_ERROR(
                    "Cannot receive an operation of type "
                    << sender_operation_type_str(expected)
                    << "; PoseidonContext is missing or invalid");
                return nullptr;
            }
        ...
        unique_ptr<SenderOperation> ZMQChannel::receive_operation(
            shared_ptr<PoseidonContext> context, SenderOperationType expected)
        {
        ...
        unique_ptr<ResultPackage> ZMQChannel::receive_result(shared_ptr<PoseidonContext> context)
        {
            throw_if_not_connected();

            bool valid_context = context && context->parameters_literal();
            if (!valid_context) {
                // Cannot receive a result package without a valid PoseidonContext
                APSI_LOG_ERROR(
                    "Cannot receive a result package; PoseidonContext is missing or invalid");
                return nullptr;
            }
