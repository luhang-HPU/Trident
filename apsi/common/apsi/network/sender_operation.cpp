...
namespace apsi {
    ...
    namespace network {
        ...
        size_t SenderOperationParms::load(istream &in, shared_ptr<PoseidonContext> context)
        {
        ...
        size_t SenderOperationOPRF::load(istream &in, shared_ptr<PoseidonContext> context)
        {
            // The context cannot be set for this operation
            if (context) {
                throw invalid_argument("context must be null");
                ...
                for (const auto &q : data) {
                    ...
                    auto cts = fbs_builder.CreateVector([&]() {
                        ...
                        for (const auto &ct : q.second) {
                            // Save each PoseidonObject<poseidon::Ciphertext>
        ...
        size_t SenderOperationQuery::load(istream &in, shared_ptr<PoseidonContext> context)
        {
