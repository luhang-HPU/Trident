...
namespace apsi {
    namespace util {
        ...
        parms_id_type get_parms_id_for_chain_idx(PoseidonContext context, size_t chain_idx)
        {
            ...
            parms_id_type parms_id = context.crt_context()->first_parms_id();
            while (context.crt_context()->get_context_data(parms_id)->chain_index() > chain_idx)
            {
                parms_id = context.crt_context()
