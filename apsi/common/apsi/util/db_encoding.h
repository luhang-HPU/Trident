    ...
    namespace util {
        ...
        */
        std::vector<felt_t> bits_to_field_elts(
            BitstringView<const unsigned char> bits, const poseidon::Modulus &mod);

        /**
        Converts the given bitstring to a sequence of field elements (modulo `mod`).
        */
        std::vector<felt_t> bits_to_field_elts(
            BitstringView<unsigned char> bits, const poseidon::Modulus &mod);

        /**
        Converts the given field elements (modulo `mod`) to a bitstring.
        */
        Bitstring field_elts_to_bits(gsl::span<const felt_t> felts, std::uint32_t bit_count,
        ...
        */
        AlgItemLabel algebraize_item_label(const HashedItem &item, const EncryptedLabel &label,
        ...
        */
        AlgItem algebraize_item(const HashedItem &item, std::size_t item_bit_count,
                                const poseidon::Modulus &mod);
        ...
        */
        HashedItem dealgebraize_item(const AlgItem &item, std::size_t item_bit_count,
                                     const poseidon::Modulus &mod);
        ...
        num_elements > 128.
        */
        EncryptedLabel dealgebraize_label(const AlgLabel &label, std::size_t label_bit_count,
                                          const poseidon::Modulus &mod);
    } // namespace util
} // namespace apsi
