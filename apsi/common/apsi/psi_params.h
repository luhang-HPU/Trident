...
public:
    ...
    scheme.
    */
    class PoseidonParams : public poseidon::ParametersLiteralDefault
    {
    public:
        PoseidonParams(uint32_t degree = 16384)
            : poseidon::ParametersLiteralDefault(SchemeType::BFV, degree)
        {
        }
    };
    ...
    */
    ...
    const ItemParams &item_params() const { return item_params_; }

    const TableParams &table_params() const { return table_params_; }

    const QueryParams &query_params() const { return query_params_; }

    const PoseidonParams &poseidon_params() const { return poseidon_params_; }

    std::uint32_t items_per_bundle() const { return items_per_bundle_; }

    std::uint32_t bins_per_bundle() const { return bins_per_bundle_; }

    std::uint32_t bundle_idx_count() const { return bundle_idx_count_; }

    std::uint32_t item_bit_count() const { return item_bit_count_; }

    std::uint32_t item_bit_count_per_felt() const { return item_bit_count_per_felt_; }

    PSIParams(const ItemParams &item_params, const TableParams &table_params,
              const QueryParams &query_params, const PoseidonParams &poseidon_params)
        : item_params_(item_params), table_params_(table_params), query_params_(query_params),
          poseidon_params_(poseidon_params)
    {
        initialize();
    }
...
private:
    ...
    PoseidonParams poseidon_params_;
