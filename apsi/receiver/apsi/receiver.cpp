...
namespace apsi {
    ...
    namespace receiver {
        ...
        {
            // Generate new keys
            key_gen_ = std::make_unique<poseidon::KeyGenerator>(*get_context());

            // Set the symmetric key, encryptor, and decryptor
            crypto_context_.set_secret(key_gen_->secret_key());

            // Create Serializable<RelinKeys> and move to relin_keys_ for storage
            relin_keys_.clear();
            if (get_context()->crt_context()->using_keyswitch())
            {
                Serializable<RelinKeys> relin_keys(key_gen_->create_relin_keys());
        ...
        void Receiver::initialize()
        {
            ...
            // Initialize the CryptoContext with a new PoseidonContext
        ...
        pair<Request, IndexTranslationTable> Receiver::create_query(const vector<HashedItem> &items)
        {
                ...
                for (uint32_t bundle_idx = 0; bundle_idx < params_.bundle_idx_count();
                    ...
                    for (auto &item : bundle_items) {
                        ...
                        vector<uint64_t> alg_item =
                            bits_to_field_elts(item_bits, params_.poseidon_params().plain_modulus());
            ...
            unordered_map<uint32_t, vector<PoseidonObject<Ciphertext>>> encrypted_powers;
            ...
            const ResultPart &result_part) const
        {
            ...
            poseidon_for_each_n(iter(plain_rp_iter, size_t(0)), items_per_bundle, [&](auto &&I) {
                ...
                if (label_byte_count) {
                    ...
                    EncryptedLabel encrypted_label = dealgebraize_label(
                        alg_label, received_label_bit_count, params_.poseidon_params().plain_modulus());
            ...
            const vector<ResultPart> &result) const
        {
            ...
            for (auto &result_part : result) {
                ...
                poseidon_for_each_n(iter(mrs, this_mrs, size_t(0)), mrs.size(), [](auto &&I) {
                    if (get<1>(I) && !get<0>(I)) {
            ...
            Channel &chl) const
        {
            ...
            while (true) {
                ...
                while (!(result_part = chl.receive_result(context)))
                    ;
