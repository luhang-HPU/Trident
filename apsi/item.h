...
namespace std {
    ...
    */
    template <>
    struct hash<oprf::Item> {
        std::size_t operator()(const oprf::Item &item) const
        {
    ...
    template <>
    struct hash<oprf::HashedItem> {
        std::size_t operator()(const oprf::HashedItem &item) const
        {
            return hash<oprf::Item>()(item);
        }
    };
} // namespace std
