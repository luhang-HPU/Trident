...
namespace APSITests {
    namespace {
        ...
        shared_ptr<PSIParams> get_params()
        {
            ...
            if (!params) {
                ...
                // poseidon_params.set_coeff_modulus(CoeffModulus::BFVDefault(pmd));
                poseidon_params.set_plain_modulus(65537);

                params =
                    make_shared<PSIParams>(item_params, table_params, query_params, poseidon_params);
            }
        ...
        shared_ptr<CryptoContext> get_context()
        {
            ...
            if (!context) {
                ...
                KeyGenerator keygen(*context->context());
    ...
    TEST_F(ZMQChannelTests, ClientServerFullSession)
    {
        ...
        auto nsop = svr.receive_network_operation(get_context()->context(), true);
        ...
        nsop = svr.receive_network_operation(get_context()->context(), true);
    ...
    TEST_F(ZMQChannelTests, MultipleClients)
    {
        ...
        thread serverth([&finished] {
            ...
            while (!finished) {
                ...
                if (!(sop = sender.receive_network_operation(get_context()->context()))) {
                    this_thread::sleep_for(50ms);
                    continue;
                }
