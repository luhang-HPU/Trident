...
namespace sender
{
void Sender::RunParams(const ParamsRequest &params_request, shared_ptr<SenderDB> sender_db,
                       network::Channel &chl, function<void(Channel &, Response)> send_fun)
{
    ...
    {
        APSI_LOG_ERROR("Failed to send response to parameter request; function threw an exception: "
                       << ex.what());
        throw;
    }
...
void Sender::RunOPRF(const OPRFRequest &oprf_request, OPRFKey key, network::Channel &chl,
                     function<void(Channel &, Response)> send_fun)
{
    ...
    APSI_LOG_INFO("Start processing OPRF request for "
                  << oprf_request->data.size() / oprf_query_size << " items");
    ...
    {
        APSI_LOG_ERROR(
            "Failed to send response to OPRF request; function threw an exception: " << ex.what());
        throw;
    }
...
void Sender::RunQuery(const Query &query, Channel &chl,
                      function<void(Channel &, Response)> send_fun,
                      function<void(Channel &, ResultPart)> send_rp_fun)
{
    ...
    // We use a custom POSEIDON memory that is freed after the query is done
    ...
    APSI_LOG_INFO("Start processing query request on database with " << sender_db->get_item_count()
                                                                     << " items");
    ...
    // query.relin_keys() simply holds an empty poseidon::RelinKeys instance. There is no
    ...
    {
        APSI_LOG_ERROR(
            "Failed to send response to query request; function threw an exception: " << ex.what());
        throw;
    }
    ...
    for (auto &q : query.data())
    {
        ...
        for (size_t bundle_idx = 0; bundle_idx < all_powers.size(); bundle_idx++)
        {
            ...
            APSI_LOG_DEBUG("Extracting query ciphertext power " << exponent << " for bundle index "
                                                                << bundle_idx);
...
void Sender::ComputePowers(const shared_ptr<SenderDB> &sender_db,
                           const CryptoContext &crypto_context,
                           vector<CiphertextPowers> &all_powers, const PowersDag &pd,
                           uint32_t bundle_idx, MemoryPoolHandle &pool)
{
    ...
    bool relinearize = crypto_context.poseidon_context()->crt_context()->using_keyswitch();
    pd.parallel_apply(
        [&](const PowersDag::PowersNode &node)
        {
            if (!node.is_source())
            {
                ...
                if (relinearize)
                {
                    evaluator->relinearize(prod, prod, *relin_keys);
                }
    ...
    auto low_powers_parms_id = get_parms_id_for_chain_idx(*crypto_context.poseidon_context(), 2);
        ...
        futures.push_back(tpm.thread_pool().enqueue(
            [&, power]()
            {
                if (!ps_low_degree)
                {
                    ...
                    evaluator->drop_modulus(powers_at_this_bundle_idx[power],
                                            powers_at_this_bundle_idx[power], high_powers_parms_id);
                    // All powers must be in NTT form
                    evaluator->ntt_fwd(powers_at_this_bundle_idx[power],
                                       powers_at_this_bundle_idx[power]);
                }
                else
                {
...
void Sender::ProcessBinBundleCache(const shared_ptr<SenderDB> &sender_db,
                                   const CryptoContext &crypto_context,
                                   reference_wrapper<const BinBundleCache> cache,
                                   vector<CiphertextPowers> &all_powers, Channel &chl,
                                   function<void(Channel &, ResultPart)> send_rp_fun,
                                   uint32_t bundle_idx, compr_mode_type compr_mode,
                                   MemoryPoolHandle &pool)
{
    ...
    if (using_ps)
    {
        rp->psi_result = matching_polyn.eval_patstock(crypto_context, all_powers[bundle_idx],
