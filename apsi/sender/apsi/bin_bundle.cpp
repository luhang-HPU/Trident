...
*/
template <typename BinT> auto get_iterator(BinT &bin, const CuckooFilter &filter, felt_t element)
{
    if (filter.contains(element))
    {
...
void try_clear_irrelevant_bits(const ParametersLiteral &parms, Ciphertext &ciphertext)
{
    ...
    if (parms.coeff_modulus().size() == 1)
    {
        ...
        int compr_coeff_bit_count = parms.plain_modulus().bit_count() +
                                    get_significant_bit_count(parms.degree())
        ...
        if (irrelevant_bit_count > 0)
        {
            ...
            poseidon_for_each_n(iter(ciphertext), ciphertext.size(),
                            [&](auto &&I)
                            {
                                // We only have a single RNS component so dereference once more
                                poseidon_for_each_n(*I, parms.degree(), [&](auto &J) { J &= mask; });
                            });
        }
    }
}
...
*/
Ciphertext BatchedPlaintextPolyn::eval(const vector<Ciphertext> &ciphertext_powers,
                                       MemoryPoolHandle &pool) const
{
#ifdef POSEIDON_THROW_ON_TRANSPARENT_CIPHERTEXT
    static_assert(true, "POSEIDON must be built with POSEIDON_THROW_ON_TRANSPARENT_CIPHERTEXT=OFF");
#endif
    ...
    auto context = crypto_context.poseidon_context();
    ...
    //
    // Because the plaintexts in batched_coeffs can be identically zero, POSEIDON should be
    // built with POSEIDON_THROW_ON_TRANSPARENT_CIPHERTEXT=OFF. We create a result ciphertext
    // that is identically zero and set its NTT form flag to true so the additions below
    // will work.
    Ciphertext result(pool);
    result.resize(*context, encode_parms_id, 2);
    result.is_ntt_form() = true;
    Ciphertext temp(pool);
    Plaintext coeff(pool);
    for (size_t deg = 1; deg < batched_coeffs.size(); deg++)
    {
        coeff.unsafe_load(*context,
                          reinterpret_cast<const poseidon_byte *>(batched_coeffs[deg].data()),
    ...
    coeff.unsafe_load(*context, reinterpret_cast<const poseidon_byte *>(batched_coeffs[0].data()),
    ...
    // irrelevant bits.
    while (result.parms_id() != context->crt_context()->last_parms_id())
    {
        evaluator->drop_modulus_to_next(result, result);
    }
    try_clear_irrelevant_bits(context->crt_context()->last_context_data()->parms(), result);
...
Ciphertext BatchedPlaintextPolyn::eval_patstock(const CryptoContext &eval_crypto_context,
                                                ...
                                                size_t ps_low_degree, MemoryPoolHandle &pool) const
{
#ifdef POSEIDON_THROW_ON_TRANSPARENT_CIPHERTEXT
    static_assert(true, "POSEIDON must be built with POSEIDON_THROW_ON_TRANSPARENT_CIPHERTEXT=OFF");
#endif
    ...
    auto context = eval_crypto_context.poseidon_context();
    auto evaluator = eval_crypto_context.evaluator();
    auto relin_keys = eval_crypto_context.relin_keys();
    bool relinearize = eval_crypto_context.poseidon_context()->crt_context()->using_keyswitch();

    auto high_powers_parms_id = get_parms_id_for_chain_idx(*crypto_context.poseidon_context(), 1);
    ...
    // end.
    Ciphertext result(pool);
    result.resize(*context, high_powers_parms_id, 3);
    ...
    for (size_t i = 1; i < ps_high_degree_powers; i++)
    {
        ...
        {
            coeff.unsafe_load(*context,
                              reinterpret_cast<const poseidon_byte *>(
        ...
        evaluator->drop_modulus(temp_in, temp_in, high_powers_parms_id);

        // The high powers are already in coefficient form
        evaluator->multiply_inplace(temp_in, ciphertext_powers[i * ps_high_degree]);
    ...
    if (relinearize)
    {
        evaluator->relinearize(result, result, *relin_keys);
    }
    ...
    {
        coeff.unsafe_load(*context,
                          reinterpret_cast<const poseidon_byte *>(batched_coeffs[j].data()),
                          batched_coeffs[j].size());

        evaluator->multiply_plain(ciphertext_powers[j], coeff, temp);
        evaluator->transform_from_ntt_inplace(temp);
        evaluator->drop_modulus(temp, temp, high_powers_parms_id);
        evaluator->add_inplace(result, temp);
    }
    ...
    for (size_t i = 1; i < ps_high_degree_powers + 1; i++)
    {
        coeff.unsafe_load(
            *context,
            reinterpret_cast<const poseidon_byte *>(batched_coeffs[i * ps_high_degree].data()),
            batched_coeffs[i * ps_high_degree].size());

        evaluator->multiply_plain(ciphertext_powers[i * ps_high_degree], coeff, temp);
        evaluator->drop_modulus(temp, temp, high_powers_parms_id);
        evaluator->add_inplace(result, temp);
    }
...
*/
BatchedPlaintextPolyn::BatchedPlaintextPolyn(const vector<FEltPolyn> &polyns, CryptoContext context,
                                             uint32_t ps_low_degree, bool compressed)
    : crypto_context(move(context))
{
    ...
    size_t plain_coeffs_chain_idx = min<size_t>(
        crypto_context.poseidon_context()->crt_context()->first_context_data()->chain_index(),
        ps_low_degree ? 2 : 1);
    auto encode_parms_id =
        get_parms_id_for_chain_idx(*crypto_context.poseidon_context(), plain_coeffs_chain_idx);
    ...
    for (size_t i = 0; i < max_deg + 1; i++)
    {
        ...
        size_t size = static_cast<size_t>(
            pt.save(reinterpret_cast<poseidon_byte *>(pt_data.data()), pt_data.size(), compr_mode));
        pt_data.resize(size);
...
const Modulus &BinBundle::field_mod() const
{
    const auto &context_data =
        crypto_context_.poseidon_context()->crt_context()->first_context_data();
...
template <>
int32_t BinBundle::multi_insert(const vector<pair<felt_t, vector<felt_t>>> &item_labels,
                                size_t start_bin_idx, bool dry_run)
{
    if (stripped_)
    {
        APSI_LOG_ERROR("Cannot insert data to a stripped BinBundle");
    ...
    for (const auto &curr_item_label : item_labels)
    {
        ...
        {
            APSI_LOG_ERROR("Attempted to insert item-label with incorrect label size "
...
template <> bool BinBundle::try_multi_overwrite(const vector<felt_t> &items, size_t start_bin_idx)
{
    if (stripped_)
    {
...
template <>
bool BinBundle::try_multi_overwrite(const vector<pair<felt_t, vector<felt_t>>> &item_labels,
                                    size_t start_bin_idx)
{
    ...
    for (const auto &curr_item_label : item_labels)
    {
        ...
        if (curr_label_size != label_size)
        {
            APSI_LOG_ERROR("Attempted to overwrite item-label with incorrect label size "
                           << curr_label_size << " (expected " << label_size << ")");
...
bool BinBundle::try_get_multi_label(const vector<felt_t> &items, size_t start_bin_idx,
                                    vector<felt_t> &labels) const
{
    if (stripped_)
    {
    ...
    for (size_t label_idx = 0; label_idx < cache_.felt_interp_polyns.size(); label_idx++)
    {
        futures.push_back(tpm.thread_pool().enqueue(
            [&, label_idx]()
            {
                ...
                BatchedPlaintextPolyn bip(interp_polyn, crypto_context_,
                                          static_cast<uint32_t>(ps_low_degree_), compressed_);
    ...
    for (size_t label_idx = 0; label_idx < label_size; label_idx++)
    {
        for (size_t bin_idx = 0; bin_idx < num_bins; bin_idx++)
        {
            futures.push_back(tpm.thread_pool().enqueue(
                ...
                {
                    // Compute and cache the matching polynomial
                    FEltPolyn fip = newton_interpolate_polyn(item_bins_[bin_idx],
                                                             label_bins_[label_idx][bin_idx], mod);
...
namespace
{
flatbuffers::Offset<fbs::FEltArray>
fbs_create_felt_array(flatbuffers::FlatBufferBuilder &fbs_builder, const vector<felt_t> &felts)
{
    auto felt_array_data = fbs_builder.CreateVector(felts);
    return fbs::CreateFEltArray(fbs_builder, felt_array_data);
}

flatbuffers::Offset<fbs::FEltMatrix>
fbs_create_felt_matrix(flatbuffers::FlatBufferBuilder &fbs_builder,
...
flatbuffers::Offset<fbs::Plaintext>
fbs_create_plaintext(flatbuffers::FlatBufferBuilder &fbs_builder, const vector<unsigned char> &pt)
{
    auto pt_data =
        fbs_builder.CreateVector(reinterpret_cast<const uint8_t *>(pt.data()), pt.size());
    return fbs::CreatePlaintext(fbs_builder, pt_data);
}

flatbuffers::Offset<fbs::BatchedPlaintextPolyn>
fbs_create_batched_plaintext_polyn(flatbuffers::FlatBufferBuilder &fbs_builder,
...
size_t BinBundle::save(ostream &out, uint32_t bundle_idx) const
{
    ...
    out.write(reinterpret_cast<const char *>(fbs_builder.GetBufferPointer()),
...
{
    auto verifier =
        flatbuffers::Verifier(reinterpret_cast<const unsigned char *>(in.data()), in.size());
    ...
    {
        APSI_LOG_ERROR("The loaded BinBundle field modulus ("
    ...
    {
        APSI_LOG_ERROR("The loaded BinBundle has "
    ...
    for (size_t bin_idx = 0; !stripped_ && (bin_idx < num_bins); bin_idx++)
    {
        ...
        {
            APSI_LOG_ERROR("The loaded BinBundle has an item bin of size "
        ...
        transform(item_bin.begin(), item_bin.end(), back_inserter(item_bins_[bin_idx]),
                  [&](auto felt_item)
                  {
#ifdef APSI_DEBUG
                      if (label_size &&
                      ...
                      {
                          APSI_LOG_ERROR("The loaded BinBundle data contains a repeated value "
    ...
    {
        APSI_LOG_ERROR("The loaded BinBundle has label size "
    ...
    for (size_t label_idx = 0; !stripped_ && (label_idx < label_size); label_idx++)
    {
        ...
        {
            APSI_LOG_ERROR("The loaded BinBundle has label data for "
        ...
        for (size_t bin_idx = 0; bin_idx < num_bins; bin_idx++)
        {
            ...
            {
                APSI_LOG_ERROR("The loaded BinBundle has at bin index "
            ...
            copy(label_bin.begin(), label_bin.end(),
    ...
    if (bb->cache())
    {
        ...
        for (flatbuffers::uoffset_t coeff_idx = 0; coeff_idx < batched_matching_polyn.size();
             coeff_idx++)
        {
            ...
            copy_bytes(batched_matching_polyn_coeff.data(), batched_matching_polyn_coeff.size(),
                       pt_data.data());
        ...
        for (size_t label_idx = 0; label_idx < label_size; label_idx++)
        {
            ...
            for (size_t bin_idx = 0; !stripped_ && (bin_idx < num_bins); bin_idx++)
            {
                ...
                {
                    APSI_LOG_ERROR("The loaded BinBundle cache has at bin index "
            ...
            if (!stripped_ &&
                (batched_interp_polyn_coeff_count != expected_batch_interp_polyn_coeff_count))
            {
                APSI_LOG_ERROR("The loaded BinBundle cache contains an incorrect number ("
                 ...
                 coeff_idx++)
            {
                ...
                copy_bytes(batched_interp_polyn_coeff.data(), batched_interp_polyn_coeff.size(),
                           pt_data.data());
