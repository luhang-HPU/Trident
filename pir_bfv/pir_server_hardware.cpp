...
void PIRServerHardware::preprocess_database()
{
    //  if (!is_db_preprocessed_) {
    //
    //    for (uint32_t i = 0; i < db_->size(); i++) {
    //      Plaintext tmp;
    //      evaluator_->ftt_fwd(db_->operator[](i), tmp,
...
void PIRServerHardware::set_database(unique_ptr<vector<Plaintext>> &&db)
{
    if (!db)
    {
...
void PIRServerHardware::set_database(const std::unique_ptr<const uint8_t[]> &bytes,
...
void PIRServerHardware::set_galois_key(uint32_t client_id, GaloisKeys galkey)
{
    galoisKeys_[client_id] = galkey;
}

PirQuery PIRServerHardware::deserialize_query(stringstream &stream)
{
    PirQuery q;
...
int PIRServerHardware::serialize_reply(PirReply &reply, stringstream &stream)
{
    int output_size = 0;
...
PirReply PIRServerHardware::generate_reply(PirQuery &query, uint32_t client_id)
{
    ...
    for (uint32_t i = 0; i < nvec.size(); i++)
    {
        ...
        //      //expanded_query[jj].is_ntt_form() = true;
        //    }
...
inline vector<Ciphertext> PIRServerHardware::expand_query(const Ciphertext &encrypted, uint32_t m,
                                                          uint32_t client_id)
{
...
inline void PIRServerHardware::multiply_power_of_X(const Ciphertext &encrypted,
                                                   Ciphertext &destination, uint32_t index)
{
...
void PIRServerHardware::simple_set(uint64_t index, Plaintext pt)
{
    if (is_db_preprocessed_)
    {
        Plaintext tmp;
...
Ciphertext PIRServerHardware::simple_query(uint64_t index)
{
...
void PIRServerHardware::set_one_ct(Ciphertext one)
{
    one_ = one;
    Ciphertext tmp;
