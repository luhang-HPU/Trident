...
PIRClientHardware::PIRClientHardware(const ParametersLiteral &enc_params,
...
int PIRClientHardware::generate_serialized_query(uint64_t desiredIndex, std::stringstream &stream)
{
...
PirQuery PIRClientHardware::generate_query(uint64_t desiredIndex)
{
...
uint64_t PIRClientHardware::get_fv_index(uint64_t element_index)
{
    return static_cast<uint64_t>(element_index / pir_params_.elements_per_plaintext);
}

uint64_t PIRClientHardware::get_fv_offset(uint64_t element_index)
{
    return element_index % pir_params_.elements_per_plaintext;
}

Plaintext PIRClientHardware::decrypt(Ciphertext ct)
{
    Plaintext pt;
...
vector<uint8_t> PIRClientHardware::decode_reply(PirReply &reply, uint64_t offset)
{
    Plaintext result = decode_reply(reply);
    return extract_bytes(result, offset);
}

vector<uint64_t> PIRClientHardware::extract_coeffs(Plaintext pt)
{
...
std::vector<uint64_t> PIRClientHardware::extract_coeffs(Plaintext pt, uint64_t offset)
{
    vector<uint64_t> coeffs;
    encoder_->decode(pt, coeffs);
...
std::vector<uint8_t> PIRClientHardware::extract_bytes(Plaintext pt, uint64_t offset)
{
...
Plaintext PIRClientHardware::decode_reply(PirReply &reply)
{
    ...
    for (uint32_t i = 0; i < recursion_level; i++)
    {
        ...
        for (uint32_t j = 0; j < temp.size(); j++)
        {
            ...
            if (ptxt.to_string() == "0")
            {
                std::cout << "decoded (and scaled) plaintext = " << ptxt.to_string() << std::endl;
            }
...
GaloisKeys PIRClientHardware::generate_galois_keys()
{
...
Plaintext PIRClientHardware::replace_element(Plaintext pt, vector<uint64_t> new_element,
                                             uint64_t offset)
{
...
Ciphertext PIRClientHardware::get_one()
{
    Plaintext pt("1");
    Ciphertext ct;
    if (pir_params_.enable_symmetric)
