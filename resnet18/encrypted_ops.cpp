    ...
    GaloisKeys &gal_keys, vector<Ciphertext> &cipher_pool, bool end)
{
    ...
    auto make_compact_weight_vec = [&](int i1, int i2, int i9) {
        vector<double> compact(static_cast<size_t>(n), 0.0);
        for (long j8 = 0; j8 < n; ++j8)
        {
            ...
            compact[static_cast<size_t>(j8)] =
                weight[i1][i2][ki * ki * i7 + ki * (j5 % ki) + j6 % ki][i8 + pi * i9];
        }
        return compact;
    };

    auto make_select_one_vec = [&](int j4) {
        vector<double> select(static_cast<size_t>(n), 0.0);
        for (int v1 = 0; v1 < ko * ho; ++v1)
        {
            for (int v2 = 0; v2 < ko * wo; ++v2)
            {
                ...
                {
                    const size_t idx =
                        static_cast<size_t>(ko * ko * ho * wo * u3 + ko * wo * v1 + v2);
                    if (ko * ko * u3 + ko * (v1 % ko) + v2 % ko == j4)
                    {
                        select[idx] = constant_weight[j4] / sqrt(running_var[j4] + epsilon);
                    }
                }
            }
        }
    ...
    for (int i9 = 0; i9 < q; ++i9)
    {
        ...
        for (int i1 = 0; i1 < fh; ++i1)
        {
            ...
            {
                Ciphertext temp;
                vector<double> compact_weight_vec = make_compact_weight_vec(i1, i2, i9);
                multiply_by_vector(ctxt_rot[i1][i2], compact_weight_vec, temp, encoder,
                                   evaluator);
                if (!has_sum)
                {
                    sum = temp;
        ...
        for (int i8 = 0; i8 < pi && pi * i9 + i8 < co; ++i8)
        {
            ...
            memory_save_rotate(temp, temp,
                               (n / pi) * (j4 % pi) - j4 % ko - (j4 / (ko * ko)) * ko * ko * ho * wo -
                                   ((j4 % (ko * ko)) / ko) * ko * wo,
                               evaluator, gal_keys);
            vector<double> select_one_vec = make_select_one_vec(j4);
            multiply_by_vector_inplace(temp, select_one_vec, encoder, evaluator);
            if (!has_total_sum)
            {
                total_sum = temp;
                has_total_sum = true;
                                ...
                                CKKSEncoder &encoder)
{
    ...
    auto make_diagonal = [&](int diagonal) {
        ...
        for (int i = 0; i < q; ++i)
        {
            const int j = i + r - 1 - diagonal;
            if (0 <= j && j < r)
            {
                weights[static_cast<size_t>(i)] = matrix[static_cast<size_t>(i * r + j)];
            }
        }
        return weights;
    };
    ...
    for (int s = 0; s < q + r - 1; ++s)
    {
        ...
        vector<double> weights = make_diagonal(s);
        multiply_by_vector_inplace(temp, weights, encoder, evaluator);
        if (!has_sum)
        {
            sum = temp;
            has_sum = true;
