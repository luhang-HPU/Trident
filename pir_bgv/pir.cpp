...
{
    enc_params = ParametersLiteralDefault(BGV, N, sec_level_type::tc128);
...
void print_pir_params(const PirParams &pir_params)
{
    ...
    std::cout << "elements per BGV plaintext: " << pir_params.elements_per_plaintext << std::endl;
    std::cout << "dimensions for d-dimensional hyperrectangle: " << pir_params.d << std::endl;
    std::cout << "number of BGV plaintexts (before padding): " << pir_params.num_of_plaintexts
              << std::endl;
    std::cout << "Number of BGV plaintexts after padding (to fill d-dimensional "
                 "hyperrectangle): "
              << prod << std::endl;
