    ...
    : enc_params_(enc_params), pir_params_(pir_params), is_db_preprocessed_(false)
{
    ...
    evaluator_ = PoseidonFactory::get_instance()->create_bgv_evaluator(*context_);
...
int PIRServer::serialize_reply(PirReply &reply, stringstream &stream)
{
    ...
    {
        evaluator_->drop_modulus(reply[i], reply[i], context_->crt_context()->last_parms_id());
        // (reply[i], reply[i]);
...
PirReply PIRServer::generate_reply(PirQuery &query, uint32_t client_id, BatchEncoder *encoder,
                                   Decryptor *decryptor)
{
    ...
    for (uint32_t i = 0; i < nvec.size(); i++)
    {
        ...
        for (uint32_t j = 0; j < query[i].size(); j++)
        {
            ...
            vector<Ciphertext> expanded_query_part =
                expand_query(query[i][j], total, client_id, encoder, decryptor);
        ...
        // Expanded_query in bgv is ntt form, that needn't transform expanded query to NTT
...
inline vector<Ciphertext> PIRServer::expand_query(const Ciphertext &encrypted, uint32_t m,
                                                  uint32_t client_id, BatchEncoder *encoder,
                                                  Decryptor *decryptor)
{
...
inline void PIRServer::multiply_power_of_X(Ciphertext &encrypted, Ciphertext &destination,
                                           uint32_t index)
{

    // depend encrypted decide if transform.
    bool transform = encrypted.is_ntt_form();
    if (transform)
    {
        evaluator_->ntt_inv(encrypted, encrypted);
    }
    ...
    if (transform)
    {
        evaluator_->ntt_fwd(destination, destination);
    }
}
