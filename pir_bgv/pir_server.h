...
public:
    ...
    std::vector<Ciphertext> expand_query(const Ciphertext &encrypted, std::uint32_t m,
                                         std::uint32_t client_id, BatchEncoder *encoder,
                                         Decryptor *decryptor);

    PirQuery deserialize_query(std::stringstream &stream);
    PirReply generate_reply(PirQuery &query, std::uint32_t client_id, BatchEncoder *encoder,
                            Decryptor * decryptor);
...
private:
    ...
    std::unique_ptr<EvaluatorBgvBase> evaluator_;
    ...
    void multiply_power_of_X(Ciphertext &encrypted, Ciphertext &destination,
                             std::uint32_t index);
};
